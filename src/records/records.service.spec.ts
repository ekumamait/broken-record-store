...
describe('RecordsService', () => {
  ...
  it('should return conflict if record already exists', async () => {
    ...
    expect(response.message).toBe('Record already exists with this artist, album, and format combination');
  });
  
  it('should return conflict if update creates a duplicate record', async () => {
    const updateRecordDto = { artist: 'New Artist' };
    const existingRecord = { artist: 'New Artist', album: 'Album', format: 'Vinyl', _id: '67e3e30a810d696976b8f05f' };
    ...
    expect(response.statusCode).toBe(HttpStatus.CONFLICT);
    expect(response.message).toBe('Update would create a duplicate record with the same artist, album, and format');
  });
